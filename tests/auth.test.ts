...
Deno.test("Password Manager - hashes and verifies passwords", async () => {
  ...
  const isInvalid = await passwordManager.verifyPassword("wrong-password", hash);
...
Deno.test("Session Token Manager - verifies session secrets", async () => {
  ...
  const isInvalid = await tokenManager.verifySessionSecret("wrong-secret", hash);
  expect(isInvalid).toBe(false);
});
...
Deno.test("Validator - accepts valid emails", () => {
  ...
  const result2 = new Validator().email("test.email+tag@domain.co.uk").getResult();
